...
CThread::CThread() {}
CThread::CThread(Sniffer *snifferObj) : sniffer(snifferObj), flag(Init) {
  LOG(sniffer->dev);
}
CThread::~CThread() {}
...
void CThread::run() {
  ...
  LOG(sniffer->dev);
  handle = pcap_open_live(sniffer->dev, BUFSIZ, -1, 1000, errbuf);
  if (handle == NULL) {
    ERROR_INFO(errbuf);
    exit(1);
  }
...
// void CThread::get_packet(u_char *args, const struct pcap_pkthdr *header,
//                          const u_char *packet) {
...
// void CThread::handle_ipv4(const u_char *packet) {
...
// void CThread::handle_ipv6(const u_char *packet) {
...
// void CThread::handle_arp(const u_char *packet) {
...
//     arp->src_mac[0],
//            arp->src_mac[1], arp->src_mac[2], arp->src_mac[3],
...
// void CThread::handle_tcp(const u_char *packet, size_t hdr_len,
//                          size_t total_len) {
...
// void CThread::handle_udp(const u_char *packet, size_t hdr_len,
//                          size_t total_len) {
...
// void CThread::handle_igmp(const u_char *packet, size_t hdr_len,
//                           size_t total_len) {
//   const u_char *payload;
//   size_t size_igmp;
//   size_t size_payload;
